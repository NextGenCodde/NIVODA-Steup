// server.js (supports token auth or Basic Auth depending on env)
...
if (!NIVODA_API || !NIVODA_USER || !NIVODA_PASS) {
  console.warn('WARNING: Missing NIVODA_API / NIVODA_USER / NIVODA_PASS in env');
}
...
app.get('/search', async (req, res) => {
  try {
    ...
    if (!cert) return res.status(400).json({ error: 'certificate query param required' });
    ...
    for (const v of variants) {
      ...
      if (total && total > 0) {
        ...
        const redirectUrl = MAP_BASE_URL ? (MAP_BASE_URL + encodeURIComponent(certNumber || item.id)) : null;
        return res.json({ found: true, variantMatched: v, redirectUrl, item, attempts });
      }
    }
